...
class TestRecoverPasswordPage:
    ...
    def test_open_recover_password_page(self, driver):
        ...
        assert browser.get_current_url() == f'{URLs.BASE_URL}{URLs.FORGOT_PASSWORD_PAGE}'

    @allure.title('ввод почты и клик по кнопке «Восстановить»')
    def test_enter_email_click_recover(self, driver):
        browser = RecoverPasswordPage(driver)
        browser.enter_email_click_recover()
        assert browser.get_current_url() == f'{URLs.BASE_URL}{URLs.RESET_PASSWORD_PAGE}' and browser.find_element(RecoverPasswordLocators.RECOVER_PASSWORD_TEXT_AFTER_REDIRECT)
